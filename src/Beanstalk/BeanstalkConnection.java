...
public class BeanstalkConnection {
	...
	public boolean isOpen() {
		System.out.println("Hola desde dentro: "+channel.isOpen());
	...
	public String readControlResponse() throws BeanstalkDisconnectedException, BeanstalkException{
		...
		int count = 0;
		//System.out.println("Vuelvo a pasar x aqui");
		while (response == null) {
			count++;
			...
			if (count > 100) {
				throw new BeanstalkException("OH Snap, nothing to read from the buffer for 1 seconds!");
			}
			...
			byte lastByte = ' ';
			//System.out.println("2 Valor count "+count+" y buff position "+buf.position());
			for (int i=0 ; i < buf.position(); i++) {
				...
				if (response == null) {
					//System.out.println("Response es null y current byte "+curByte+" StringBuf "+stringBuf);
